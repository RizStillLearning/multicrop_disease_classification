...
class DenseNet121FeatureExtractor(nn.Module):
    '''
    Wraps a trained DenseNet-121 and returns 1024-d feature vectors.

    Forward pass:  features block → BN + ReLU → AdaptiveAvgPool2d(1) → flatten
    The classifier head is bypassed entirely.
    '''

    def __init__(self, backbone):
        super().__init__()
        self.features = backbone.features       # Sequential of DenseBlocks + transitions
...
def extract_features_from_ensemble(feature_extractors, dataloader, device):
    ...
    with torch.no_grad():
        for images, labels in dataloader:
            ...
            # Soft average across folds → (B, 1024)
...
def main():
    ...
    # Load all backbone folds and wrap as feature extractors
    feature_extractors = []
    ...
    plt.ylabel('True')
    plt.title('DenseNet-121 + SVM Confusion Matrix')
