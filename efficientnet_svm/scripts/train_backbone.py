...
def main():
    ...
    config_name = 'efficientnet_svm/config.yaml'
    ...
    for i, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(y_numpy)), y_numpy)):
        ...
        model = build_model(num_classes=num_classes, use_cbam=False)
        model.to(device)

        cur_epoch = 1
        best_val_loss = float('inf')
        best_val_acc = float('-inf')
        best_model_state = build_model(num_classes=num_classes, use_cbam=False)
    ...
    for i in range(k_fold):
        ...
        backbone = build_model(num_classes, use_cbam=False)
